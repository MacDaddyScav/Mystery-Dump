...
RegisterNetEvent('erp-shops:sendConfig')
AddEventHandler('erp-shops:sendConfig', function(Cfg)
		...
		while hasGotConfig do
			...
			if not found then
				for i=1, #weashop_locations do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "weaponshop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "weaponshop"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Weapon Shop')
						end
				...
				for i=1, #JobShops do
					...
					if shouldPass then
						if #(coords - pos) < 2.0 then
							found = true
							if promptStatus == 1 and promptType ~= "jobshops" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
							if promptStatus == 0 then
								promptStatus = 1
								promptType = "jobshops"
								TriggerEvent('erp-prompts:ShowUI', 'show', JobShops[i]['message'] or '[E] Access Shop')
							end
							if IsControlJustPressed(1, 38) then
			...
			if not found then
				if #(vector3(-494.07, -342.79, 42.3) - pos) < 3.0 then
					found = true
					if promptStatus == 1 and promptType ~= "emsshop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "emsshop"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Access Shop')
					end
			...
			if not found then
				if #(vector3(1826.57, 3686.51, 34.27) - pos) < 3.0 then
					found = true
					if promptStatus == 1 and promptType ~= "cmmcshop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "cmmcshop"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Access Shop')
			...
			if not found then
				if #(vector3(243.28, -1090.52, 29.28) - pos) < 3.0 then
					found = true
					if promptStatus == 1 and promptType ~= "idshop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "idshop"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Collect ID')
					end
					if IsControlJustPressed(1, 38) then
						local PlayerData = exports['echorp']:GetPlayerData()
						...
						local keyboard = exports["erp-dialog"]:KeyboardInput({
								header = "Mugshot URL", 
						...
						if keyboard and keyboard[1] then
							...
							if tmpInput == "" or tmpInput == nil then
								exports['mythic_notify']:SendAlert('inform', 'You must provide a link that ends with .png or .jpg')
			...
			if not found then
				for i=1, #twentyfourseven_shops do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "twentyfourseven_shops" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "twentyfourseven_shops"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Browse Store')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then -- EMS Public Shop
				if #(vector3(-433.28, -340.56, 34.91) - pos) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "emspublic" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
			...
			if not found then -- JaiL Food
				if #(vector3(1783.44, 2558.97, 45.66) - pos) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "22shop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "22shop"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Access Shop')
			...
			if not found then
				for i=1, #police_armory do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "police_armory" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "police_armory"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Police Armory')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				for i=1, #tool_shops do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "toolstore" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "toolstore"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Tool Store')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				for i=1, #electronic_shops do
					...
					if dist < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "electronic_shops" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "electronic_shops"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Electronics Store')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				for i=1, #mechanic_parts_shops do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "mechanic_parts_shops" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "mechanic_parts_shops"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Mechanic Store')
						end
						if IsControlJustPressed(1, 38) then
							if exports["erp-inventory"]:IsMechanic() then
			...
			if not found then
				...
				if #(sexStore - pos) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "sexStore" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "sexStore"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Sex lol')
					end
					if IsControlJustPressed(1, 38) then
			...
			if not found then
				for i=1, #SecretStashes do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "SecretStashes" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptStatus = "SecretStashes"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Open Stash')
						end
						if IsControlJustPressed(1, 38) then
							if SecretStashes[i]['job'] == 'nothing' then
								...
								if (GetOnscreenKeyboardResult()) then
									...
									if not result or result == nil then
										exports['mythic_notify']:SendAlert('inform', 'Hurrrr, you provided no code :(', 5000)
									else
										if tostring(SecretStashes[i]['code']) == tostring(result) then
											...
											exports['mythic_notify']:SendAlert('error', 'Wrong code', 5000)
										end
									end
								end
			...
			if not found then
				for i=1, #Stashes do
					...
					if #(coords - pos) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "Stashes" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "Stashes"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Open Stash')
						...
						if IsControlJustPressed(1, 38) then
							if currentJob == Stashes[i]['job'] and exports["isPed"]:isPed("grade") >= Stashes[i]['grade'] then
								...
								exports['mythic_notify']:SendAlert('inform', 'Access denied.')
								Wait(1000)
							end
						end
					end
			...
			if not found then
				if #(pos - vector3(472.71, -992.73, 26.26)) < 2.0 then
					found = true
					if promptStatus == 1 and promptType ~= "mrpdstash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
				...
				elseif #(pos - vector3(1858.05, 3692.36, 34.27)) < 2.0 then
					found = true
					if promptStatus == 1 and promptType ~= "sheriffstationstash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
				...
				elseif #(pos - vector3(1844.0, 2574.3, 46.01)) < 2.5 then
					found = true
					if promptStatus == 1 and promptType ~= "prisonstash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
				...
				elseif #(pos - vector3(387.1949, 796.5566, 187.6775)) < 2.5 then
					found = true
					if promptStatus == 1 and promptType ~= "sapr-stash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
			...
			if not found then
				if #(pos - vector3(486.53, -995.41, 30.68)) < 2.5 then
					found = true
					if promptStatus == 1 and promptType ~= "mrpdtrash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "mrpdtrash"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] MRPD Trash')
					end
					if IsControlJustPressed(1, 38) then
						if job.isPolice then
				...
				elseif #(pos - vector3(1838.68, 2576.76, 46.01)) < 2.5 then
					found = true
					if promptStatus == 1 and promptType ~= "prisontrash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "prisontrash"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Prison Trash')
					end
					if IsControlJustPressed(1, 38) then
						if job.isPolice then
			...
			if not found then
				if #(pos - vector3(-436.69, -318.08, 34.91)) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "emsstash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "emsstash"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] EMS Stash')
					end
					if IsControlJustPressed(1, 38) then
			...
			if not found then
				if #(pos - vector3(1822.88, 3666.58, 34.27)) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "cmmcstash" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "cmmcstash"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] CMMC Stash')
					end
					if IsControlJustPressed(1, 38) then
			...
			if not found then
				if #(pos - vector3(-633.22, 235.48, 81.87)) < 1.0 then
					found = true
					if promptStatus == 1 and promptType ~= "coffeeshop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "coffeeshop"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Coffee Shop')
					end
					if IsControlJustPressed(1, 38) then
			...
			if not found then
				if currentJob == 'vanilla' then 
					if #(pos - vector3(129.9, -1284.36, 29.26)) < 1.0 then
						found = true
						if promptStatus == 1 and promptType ~= "vu" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "vu"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Vanilla Unicorn')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				if currentJob == 'cdmortuary' then
					if #(pos - vector3(248.94, -1374.94, 39.52)) < 1.5 then
						found = true
						if promptStatus == 1 and promptType ~= "morgueShop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "morgueShop"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Morgue Shop')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				if currentJob == 'potato' then
					...
					if #(pos - potCoords) < 2.0 then
						found = true
						if promptStatus == 1 and promptType ~= "ProcessPotato" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "ProcessPotato"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Process Potato')
						end
						if IsControlJustPressed(1, 38) then
			...
			if not found then
				for i=1, #ShopCounters do
					...
					if #(coords - pos) < 1.1 then
						found = true
						if promptStatus == 1 and promptType ~= "ShopCounters" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "ShopCounters"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Open Counter')
						end
						if IsControlJustPressed(1, 38) then
							if string.find(Stashes[i]['name'], "storage") then
			...
			if not found then
				...
				if #(pos - fuckthis) < 2.0 then
					found = true
					if promptStatus == 1 and promptType ~= "baggedpuremeth" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "baggedpuremeth"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Craft')
					end
					if IsControlJustPressed(1, 38) then
						if exports["erp-inventory"]:hasEnoughOfItem("rawmethpile",1,false) then
							if exports["erp-inventory"]:hasEnoughOfItem("ziplock_bag",10,false) then
								TriggerEvent('dp:playEmote', "mechanic4")
								local finished = exports["erp-progressbar"]:taskBar(5000,"Bagging Pure Meth",false,false)
								if (finished == 100) then
									if exports["erp-inventory"]:hasEnoughOfItem("rawmethpile",1,false) and exports["erp-inventory"]:hasEnoughOfItem("ziplock_bag",10,false) then
								...
								exports['mythic_notify']:SendAlert('inform', 'Missing bags.')
							end
						else
							exports['mythic_notify']:SendAlert('inform', 'Missing an essential meth product.', 5000)
						end
						Wait(1000)
					end
				end
			...
			if not found then
				...
				if #(pos - fuckthis) < 2.0 then
					found = true
					if promptStatus == 1 and promptType ~= "refinedmeth" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
					...
					if IsControlJustPressed(1, 38) then
						local baggedpuremeth = exports['erp-inventory']:getQuantity('baggedpuremeth')
						local toTake = math.random(5, 8)
						if baggedpuremeth >= toTake then
							TriggerEvent('dp:playEmote', "mechanic4")
							local finished = exports["erp-progressbar"]:taskBar(5000,"Refined Meth Bags",false,false)
							if finished == 100 then 
								if exports["erp-inventory"]:hasEnoughOfItem("baggedpuremeth",toTake,false) then
							...
							exports['mythic_notify']:SendAlert('inform', 'Missing pure meth.')
						end
						Wait(1000)
					end
				end
			...
			if not found then
				...
				if #(pos - fuckthis) < 2.0 then
					found = true
					if promptStatus == 1 and promptType ~= "thermalcharge" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
					if promptStatus == 0 then
						promptStatus = 1
						promptType = "thermalcharge"
						TriggerEvent('erp-prompts:ShowUI', 'show', '[?] ???')
					end
					if IsControlJustPressed(1, 38) then
			...
			if not found then
				...
				if currentJob == 'weazelnews' then
					if #(pos - fuckthis) < 2.0 then
						found = true
						if promptStatus == 1 and promptType ~= "WeazelShop" then TriggerEvent('erp-prompts:HideUI') Wait(100) end;
						if promptStatus == 0 then
							promptStatus = 1
							promptType = "WeazelShop"
							TriggerEvent('erp-prompts:ShowUI', 'show', '[E] Weazel Shop')
						end
						if IsControlJustPressed(1, 38) then
...
AddEventHandler('echorp:playerSpawned', function(sentData) 
	Wait(5000)
	if exports["erp-inventory"]:IsMechanic() then TriggerEvent('mechanicshopblipthing') end
end)
...
RegisterNUICallback('getCodeResult', function(data, cb)
  ...
  if code == "" then exports['mythic_notify']:SendAlert('inform', 'Please enter a code.') return end
